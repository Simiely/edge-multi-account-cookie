...
**删除同步（墓碑机制）：已删除的账号不会再被旧备份同步复活**

- **背景问题**：快照式同步中"删除"无法传播——本地删除账号后，远端旧备份还有该账号，下次「一键同步」又被拉回来
- **墓碑方案（业界标准，参考 Storyie/DataStax/Cassandra）**：
  - 删除改为**软删除**：保留账号骨架（`deleted:true` + `deletedAt` 时间戳），清空 cookies/localStorage——删除成为"可观察、可同步"的变化
  - **合并规则**（smart 合并加墓碑分支）：远端活跃 vs 本地墓碑 → 远端 `updatedAt > deletedAt` 才**复活**（删除后又保存过新数据），否则保持墓碑；远端墓碑 vs 本地活跃 → 本地更新则保留、否则**删除传播**到本地；远端墓碑 vs 本地无 → 导入墓碑（幂等）
  - **墓碑 TTL**：保留 30 天后物理清理（`purgeOldTombstones`，保存/同步时顺带执行），避免无限累积
  - **全链路过滤**：账号列表 / 统计数 / 右键菜单 / 每日体检全部跳过墓碑（`getDomainAccounts` 统一过滤）
- 导出/备份包含墓碑 → 同步时删除标记传播到其他设备（端到端验证：本地删除 → 同步 → 远端备份携带墓碑、删除不复活）
- UI：删除提示改为"已删除，将在下次同步时同步到其他设备"；同步结果新增"复活 N / 删除同步 N"计数
- 删除 `deleteDomainAccounts` 死函数
...
- 新增 `webdav.sync` action：**先拉后传**双向收敛——① 下载远端数据最新备份（内容感知筛选，v2.7.5）→ smart 合并进本地（同名账号取 updatedAt 最新，本地独有保留）；② 导出合并后的本地全量 → 上传新文件（保留策略自动清理旧文件，数据已含全部最新，不丢）
- **解决上传方向裸奔问题**：此前上传不比对，本地旧数据可能"覆盖"掉远端新数据；同步的先拉后传 + smart 合并天然无损——远端更新的账号先进本地，上传时必然包含全部最新
- UI：设置页"立即备份/下载恢复"两按钮合并为「🔄 一键同步」；弹窗底部"上传/下载"合并为「同步」；同步结果提示"拉取：新增 X 更新 Y 保留 Z；上传：xxx.json"
- **测试保存**：设置页"测试连接 + 保存配置"两按钮合并为「🔌 测试保存」——测试连接成功即自动保存配置（密码留空保留已存）；测试失败不保存并提示；URL/账号/密码输入框回车 = 触发测试保存
- **移除保留份数与自动备份**：删除设置页高级选项（保留份数配置 + 每日/每周自动备份计划）；远端固定保留最新 1 份（上传自动清理旧文件）；删除 `webdav-backup` 定时 alarm 与 `runWebdavBackup`；备份仅由「一键同步」手动触发；每日会话体检（健康标记）保留不受影响
- 删除不再使用的 `webdav.push` / `webdav.pull` / `webdav.preview` action 与 `diffBackup` 死函数（action 表精简至 13 个全在用）
- 首次同步（远端无备份）自动跳过拉取直接上传首份
- 自动备份计划（定时 WebDAV 备份）保持独立语义：定时快照上传，不受同步按钮影响
...
- `webdavPull` 不再"按文件名排序取最后一份"，改为**逐个下载所有备份，优先按文件内 `__meta.exportedAt`（数据导出时间）判定新旧**——手动上传的旧数据备份（文件名新、内容旧）不会再被误选
- 旧格式备份（v2.7.3 之前无元数据）或解密失败时，回退用文件名时间戳（UTC）参与比较；单个文件损坏/下载失败自动跳过，认证失败整体终止
- 返回 `totalBackups`（扫描份数）；下载恢复预览弹窗显示"🔍 已自动筛选 N 份备份，选中数据最新的一份"
- 与 v2.7.4 智能合并叠加：**选最新文件 → 差异核对 → 同名账号取最新版本**，WebDAV 全链路零手动选择

## v2.7.4（当前版本）

**导入/恢复简化：智能合并（同名账号自动取最新）**

- 移除导入模式选择（合并/覆盖分段控件）——导入本地备份、WebDAV 下载恢复均默认**智能合并**：同一域名下同名账号比较 `updatedAt`，自动采用更新的那份（本地新 → 保留本地；远端新 → 采用远端），双向同步不丢数据
- `importData` 默认 `smart` 模式（保留 `replace` 兼容内部旧逻辑）；返回 `{imported, updated, skipped}`，UI 提示"新增 X / 更新 Y / 保留 Z 个本地更新"
- 设置页本地备份卡片、WebDAV 下载恢复、弹窗下载按钮同步简化
