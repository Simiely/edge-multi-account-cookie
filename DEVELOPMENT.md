...
### 38. 清空本地 ≠ 删除：清空仅本机，删除才传播（v2.11.2 P1 → v2.11.4 语义修正）

**TL;DR**：「清空本地账号数据」与「逐账号删除」是**两种不同语义的用户意图**，必须严格区隔：
- **删除账号**（deleteAccount）= 该账号在所有设备都不要 → 走墓碑机制（§36）跨设备传播；
- **清空本地**（data.clearAll）= 丢弃本机副本、之后想从网络端同步恢复 → **仅本机物理清空，不生成墓碑、不传播删除**。

**历史**：v2.11.2 曾因"清空物理删库被远端拉取复活 / 空上传覆盖远端备份"把清空改为**墓碑化全部账号**（`deleted:true + deletedAt:now`）→ 导致用户「清空本地 → 同步」后远端备份也被墓碑覆盖、其他设备全部清空——**用户实测反馈不符预期**：清空本地的意图是从网络端恢复，而非删除所有设备。**v2.11.4 修正**：`data.clearAll` 改回**仅本机物理清空**（`storage.local.remove(STORAGE_KEY)`），不生成墓碑；**§38 ② 的教训通过 `webdav.sync` 上传前兜底保留**：合并后本地条目（含墓碑）为 0 时跳过上传（`pushed=null`），杜绝"清空 + 远端无备份"把空数据上传覆盖远端备份。**坑**：① 清空是本地操作——远端有备份时同步会拉回（恢复本地）；远端无备份时跳过上传（不覆盖远端）；② 切勿把"清空"与"删除"混用：清空走物理删库（无墓碑）、删除走墓碑传播；③ popup/设置页同步提示已处理 `pushed=null`（显示"本地无数据，未上传"，不再 TypeError 崩溃）；④ **升级迁移坑（用户实测）**：从 v2.11.2 升到 v2.11.4 时，若本地残留旧版"墓碑化清空"留下的墓碑（小文件、几 KB），且**未先清空本地**直接同步——合并时本地墓碑 `deletedAt` 比远端账号 `updatedAt` 新 → 墓碑压过远端活跃数据 → 导出上传的是墓碑 → **远端完整备份被小墓碑覆盖**（用户实测：459KB 备份被同步成 5.6KB）。**升级后必须先「清空本地」再同步**，让本地墓碑清掉后从远端重新拉取恢复。

### 39. 墓碑物理清理必须晚于上传（v2.11.3 P0 修复，墓碑 TTL 过期同步复活）
