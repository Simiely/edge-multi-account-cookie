...
async function importData(encryptedBlob, pin, mode = 'smart') {
  ...
  // v2.11.3 修复：墓碑不在导入合并时物理清理。
  // 原实现此处 purgeOldTombstones 直接删除过期墓碑，早于 webdav.sync 的第二步上传——
  // 墓碑在"传播到远端备份"之前就被清掉 → 导出/上传不含墓碑 → 远端备份被覆盖丢失删除标记 →
  // 其他设备（本地有旧账号）同步时墓碑"复活"（DEVELOPMENT §36 语义：墓碑须存活足够久传播删除）。
  // 墓碑的物理清理改由 webdav.sync 上传成功后执行（墓碑先写入远端，再清本地），见 handlers/webdav.js。
