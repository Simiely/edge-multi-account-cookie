...
async function importData(encryptedBlob, pin, mode = 'smart') {
  ...
  for (const domain of Object.keys(accountsIn)) {
    ...
    for (const name of Object.keys(accountsIn[domain])) {
      const incomingEntry = accountsIn[domain][name];
      const existing = data.accounts[domain][name];
      if (!existing) {
        data.accounts[domain][name] = incomingEntry;
        imported++;
        continue;
      }
      // 同名：取 updatedAt 更新的（远端新 → 覆盖本地；本地新/相同 → 保留本地）
      const inU = (incomingEntry && incomingEntry.updatedAt) || 0;
