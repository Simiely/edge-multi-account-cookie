 ...
 * 版本迁移管线（惰性：每次读取时检测）。
 * v2 → v3：
 *   - 若存在未加密的 cookie.value（v2 明文），用主密钥加密
...
async function migrate(data) {
  ...
  if (data.version < 3) {
    try {
      const mk = await getMasterKey(); // 确保 MK 存在
      const accounts = data.accounts || {};
      // 逐条加密明文 cookie value（map 内不能 await，故用 for 循环）
      for (const domain of Object.keys(accounts)) {
        for (const name of Object.keys(accounts[domain])) {
          ...
          for (const c of entry.cookies) {
            if (typeof c.value === 'string' && c.value.startsWith('enc:')) { out.push(c); continue; }
            const plain = String(c.value || '');
            if (plain === '') { out.push(c); continue; }
            const encValue = await encryptWithKey(plain, mk);
            out.push({ ...c, value: 'enc:' + encValue, partitionKey: c.partitionKey || undefined, storeId: c.storeId || undefined });
          }
          entry.cookies = out;
        }
      }
